...
class Weather():
    def __init__(self):
        self.city_name = "基隆市"
        self.t = str(out[0])
        self.Month_state=t.split()
        
    def print_city(self):
        return self.Month_state[order[0]]
    
    def print_night_temperature(self):#今晚到明晨溫度range
        return self.Month_state[order[7]]+Month_state[order[2]]
    
    def print_day_temperature(self):#明日白天溫度range
        return self.Month_state[order[13]]+Month_state[order[2]]
    
    def print_night_rain_percent(self):#今晚到明晨降雨機率
        return self.Month_state[order[10]]+Month_state[order[11]]
    
    def print_day_rain_percent(self):#明日白天降雨機率
        return self.Month_state[order[16]]+Month_state[order[17]]
